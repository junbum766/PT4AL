...
checkpoint = torch.load('./checkpoint_4/rotation.pth') #####
...
def test(epoch):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, inputs1, inputs2, inputs3, targets, targets1, targets2, targets3, path) in enumerate(testloader):
            ...
            with open('./rotation_loss_4.txt', 'a') as f: #####
                f.write(s)
...
if __name__ == "__main__":
    test(1)
    with open('./rotation_loss_4.txt', 'r') as f: #####
        losses = f.readlines()
